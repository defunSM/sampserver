...
#define ADMIN_TELEPORT 80
#define ADMIN_CASH 70
#define ADMIN_STAT 56
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 50 && response == 1)
	{
        ...
        case 6: ShowPlayerDialog(playerid, ADMIN_STAT, DIALOG_STYLE_LIST, "{00C0FF}Stats", "Reset XP\n Set XP", "Select", "Cancel");
		}

	}
	
	if(dialogid == ADMIN_STAT && response == 1){
	    switch(listitem){
	...
	if(dialogid == 57 && response == 1) {
		...
		PlayerInfo[Assign[temp01]][pEXP] = 0;
		ChangeEXP(Assign[temp01]);
	...
	if(dialogid == 59 && response == 1) {
		...
		PlayerInfo[Assign[temp01]][pEXP] = Assign[temp02];
		ChangeEXP(Assign[temp01]);
	...
	if(dialogid == 60 && response == 1){
		...
		case 1: ShowPlayerDialog(playerid, ADMIN_CASH, DIALOG_STYLE_INPUT, "Cash", "Target ID:", "Confirm", "Cancel");
		case 2: ShowPlayerDialog(playerid, ADMIN_TELEPORT, DIALOG_STYLE_INPUT, "Teleport","Enter Target to teleport to:", "Confirm", "Cancel" );
	...
	if(dialogid == ADMIN_TELEPORT && response == 1)
	{
		GetPlayerPos(strval(inputtext), PlayerInfo[playerid][pX], PlayerInfo[playerid][pY], PlayerInfo[playerid][pZ]);
		SetPlayerPos(playerid, PlayerInfo[playerid][pX], PlayerInfo[playerid][pY], PlayerInfo[playerid][pZ]);
		new string[129];
		format(string, sizeof(string), "[ADMIN] You have teleported yourself to ID: %d", strval(inputtext));
	...
	if(dialogid == ADMIN_CASH && response == 1){
		Assign[temp01]=strval(inputtext);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(PRESSED(KEY_JUMP) && PlayerInfo[playerid][pRace] == 1)
	{
		...
		GetPlayerHealth(playerid, Assign[temp04]);
...
public JumpEnable(playerid) {
	PlayerInfo[playerid][pVampireJump]=1;
	SetPlayerHealth(playerid, Assign[temp04]);
}
...
{
	if(PlayerInfo[playerid][pEXP]==PlayerInfo[playerid][pMaxEXP])
	{
	    SendClientMessage(playerid, -1, "[LEVELUP] Your Character Has Gained A Level. "COL_CYAN"+1 ABILITY POINT");
	    
	}
	else
	{
	    SendClientMessage(playerid, COLOR_CYAN, "ERROR: Your character does not have enough XP");
	}
	return 1;
}
...
DisplayBars(playerid)
{
    ...
    SetPlayerProgressBarValue(playerid, PlayerBar:pEXPBarID[playerid], PlayerInfo[playerid][pEXP]);
    ShowPlayerProgressBar(playerid, PlayerBar:pEXPBarID[playerid]);
...
ChangeEXP(playerid)
{
	...
	SetPlayerProgressBarValue(0, PlayerBar:pEXPBarID[playerid], PlayerInfo[playerid][pEXP]);
}
